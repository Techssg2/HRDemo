angular
  .module("ssg.module.academy.training-invitation.all-items.ctrl", [
    "kendo.directives",
  ])
  .controller(
    "TrainingInvitationAllItemsController",
    function (
      $rootScope,
      $scope,
      $stateParams,
      ...
      $scope.title = $translate.instant("ALL_TRAINING_INVITATION_REQUEST");
      $scope.currentQuery = {
      ...
      $scope.typeofStatus = {
        ...
        $('#mySelect').on('change', function () {
            ...
            if (selectedValue === "1") {
                $scope.selectedTab = 1;
                $state.go('home.academy-myInvitation')

            } else if (selectedValue === "0") {
                $state.go('home.academy-allInvitation')
            }
        });
      ...
      async function getInvitationList(option) {
        ...
        if (option) {
          let result = await TrainingInvitationService.listAllInvitations(
            $scope.currentQuery
          ).$promise;
          if (result) {
