        ...
        try {
          ...
          if ($scope.id) {
            ...
            let workflowInstances =
              ...
              var reques = workflowInstances.histories.find(item =>
                  item.roundNumber === 1 && item.stepNumber === 0
              );

              if (reques) {
                  $scope.requester = {
                      requesterName: reques.createdByFullName,
                      dateOfSubmission: reques.created,
                  };
              } else {
                  $scope.requester = {
            ...
            if (workflowInstances) {
              ...
              $scope.workflowInstances1.workflowData.steps = $scope.workflowInstances1.workflowData.steps.map(step => {
                  let history = $scope.workflowInstances1.histories.find(h => h.stepNumber === step.stepNumber);
                  if (history) {
