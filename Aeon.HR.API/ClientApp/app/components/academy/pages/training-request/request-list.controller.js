    ...
    function (
      ...
      $scope.realTitle =
        ...
        $('#mySelect').on('change', function () {
            ...
            if (selectedValue === "1") {
                $scope.selectedTab = 1;
                $state.go('home.academy-myRequests')

            } else if (selectedValue === "0") {
                $state.go('home.academy-allRequests')
            }
        });
      ...
      $scope.trainingTypesOptions = {
        ...
        $scope.exportToExcel = function () {
            var grid = $("#gridTrainingRequests").data("kendoGrid");
            if (grid) {
                grid.saveAsExcel();
            }
        };
      this.$onInit = async function () {
        $scope.advancedSearchMode = false;
          ...
          if ($state.current.name === 'home.academy-myRequests') {
              $scope.selectedTab = "1";
          }
          else if ($state.current.name === 'home.academy-allRequests') {
              $scope.selectedTab = "0";
          }
