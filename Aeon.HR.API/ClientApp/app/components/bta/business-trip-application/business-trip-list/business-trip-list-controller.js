var ssgApp = angular.module('ssg.businessTripListModule', ["kendo.directives"]);
ssgApp.controller('businessTripListController', function ($rootScope, $scope, $location, appSetting, localStorageService, $stateParams, $state, moment, Notification, settingService, $timeout, cbService, masterDataService, workflowService, ssgexService, fileService, commonData, $translate, attachmentService, attachmentFile, btaService) {
    ...
    this.$onInit = function () {
        $scope.title = $translate.instant($stateParams.action.title).toUpperCase();
        ...
        if ($state.current.name === 'home.business-trip-application.myRequests') {
            $scope.selectedTab = "1";
            $scope.title = $translate.instant('BTA_MY_REQUESTS').toUpperCase();
        }
        else if ($state.current.name === 'home.business-trip-application.allRequests') {
            $scope.selectedTab = "0";
            $scope.title = $translate.instant('BTA_ALL_REQUESTS').toUpperCase();
        }
    }
    $scope.onTabChange = function () {
        if ($scope.selectedTab === "1") {
            $state.go('home.business-trip-application.myRequests');
        } else if ($scope.selectedTab === "0") {
            $state.go('home.business-trip-application.allRequests');
        }
    };
    ...
    $scope.export = async function () {
        ...
        var res = await fileService.getInstance().processingFiles.export({
            type: commonData.exportType.BUSINESSTRIPAPPLICATION
        }, args).$promise;
        if (res.isSuccess) {
    ...
    async function getBTAList(option) {
        $scope.data = [
        ];
        ...
        if (option) {
            let result = await btaService.getInstance().bussinessTripApps.getList($scope.currentQuery).$promise;
