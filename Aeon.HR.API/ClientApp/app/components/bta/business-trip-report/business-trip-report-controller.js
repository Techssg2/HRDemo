...
ssgApp.controller('businessTripReportController', function ($rootScope, $scope, $location, appSetting, localStorageService, $stateParams, $state, moment, Notification, settingService, $timeout, cbService, masterDataService, workflowService, ssgexService, fileService, commonData, $translate, attachmentService, attachmentFile, btaService, $compile) {
    ...
    this.$onInit = function () {
        ...
        $scope.columnGridReport = {
            ...
            reportFlightNumbers: [
                ...
                },
                {
                    ...
                    template: function (dataItem) {
                        return `<a href="javascript:void(0)" ng-click="reportOfHotel(dataItem, 'flightNumber')">${dataItem.totalStaffs}</a>`;
                    }
                }
            ],
            reportStatus: [
                ...
                },
                {
                    ...
                    width: "100px",
                    template: function (dataItem) {
                        return `<a href="javascript:void(0)" ng-click="reportOfHotel(dataItem, 'status')">${dataItem.totalRequest}</a>`;
                    }
                }
            ],
            reportFlightsBooking: [
                ...
                {
                    field: "reasonforBusinessTripDetail",
                    headerTemplate: $translate.instant('COMMON_REQUESTOR_NOTE_DETAIL'),
                    width: 250
                },
                {
            ...
            reportOverBudget: [
                ...
                {
                    field: "btA_ReferenceNumber",
                    headerTemplate: $translate.instant('BTA_REFERENCE_NUMBER'),
                    width: "180px",
                    locked: false,
                    template: function (dataItem) {
                        return `<a ui-sref="home.business-trip-application.item({referenceValue: '${dataItem.btA_ReferenceNumber}', id: '${dataItem.btaId}'})" ui-sref-opts="{ reload: true }">${dataItem.btA_ReferenceNumber}</a>`;
                    }
                },
                {
                    field: "deptName",
                ...
                },
                {
                    ...
                    width: "240px",
                    template: function (dataItem) {
                        return moment(dataItem.fromDate).format(appSetting.sortDateFormat);
                    }
                },
                {
                    field: "toDate",
                    ...
                    width: "240px",
                    template: function (dataItem) {
                        return moment(dataItem.toDate).format(appSetting.sortDateFormat);
                    }
                },
                {
                    field: "oldBudget",
                    headerTemplate: $translate.instant('BTA_OVER_OLD_BUDGET'),
                    format: "{0:n0}",
                    sortable: false,
                    width: "240px"
                },
                ...
                {
                    field: "DeviantBudget",
                    headerTemplate: $translate.instant('BTA_OVER_Deviant_Budget'),
                    format: "{0:n0}",
                    sortable: false,
                    width: "240px",
                    template: function (dataItem) {
                        return `<span>{{${dataItem.newBudget - dataItem.oldBudget}| number}}</span>`;
                    }
                },
                {
                    field: "status",
        ...
        $scope.reportTypeOptions = {
            ...
            dataSource: {
                ...
                {
                    name: $translate.instant('BTA_OVER_BUDGET'),
                    value: 5
                }
                ]
            },
            change: function (e) {
                ...
                // tudm: Over Budget
                else if ($scope.reportType == appSetting.btaReportType.OVERBUDGET) {
                    $scope.data = [];
                    $scope.getDepartments();
                    grid.setOptions({
                        ...
                        sortable: false,
                        columns: $scope.columnGridReport.reportOverBudget,
                ...
                return {
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            width: "150px",
                            template: function (dataItem) {
                                return `<a href="javascript:void(0)" ng-click="detailUserInRoom(dataItem, '${hotel.code}')">${dataItem.totalStaffs}</a>`;
                            }
                        },
                        {
                            field: "totalDays",
    ...
    async function getListReports(option, reportType) {
        ...
        //TUDM: over budget
        else if (reportType == appSetting.btaReportType.OVERBUDGET) {
            {
                if (option) {
                    queryArgs.limit = option.data.take;
                    queryArgs.page = option.data.page;
        ...
        if (option) {
            ...
            if (reportType == appSetting.btaReportType.FLIGHTSBOOKING || reportType == appSetting.btaReportType.OVERBUDGET) {
    ...
    $scope.reportOfHotel = async function (dataItem, reportType) {
        $rootScope.showLoading(); $rootScope.hideLoading();
    ...
    function genarateQueryforReportDetail() {
        ...
        switch ($scope.reportType) {
            ...
            ///// tudm
            case appSetting.btaReportType.OVERBUDGET: {
                createType = ["fromDate", "toDate"];
                args.predicate = `(sapCode.contains(@0) || fullName.contains(@0) || departmentName.contains(@0))`;
                args.predicateParameters.push(currentStatus);
            }
        }
        if ($scope.searchInfo.fromDate && isValidDate($scope.searchInfo.fromDate)) {
    ...
    function buildArgForGetListReport(stutusName) {
        ...
        if ($scope.reportType == appSetting.btaReportType.OVERBUDGET) {
            if ($scope.searchInfo.fromDate && isValidDate($scope.searchInfo.fromDate)) {
                option.predicate = option.predicate ? option.predicate + ` && created >= @${option.predicateParameters.length}` : `created >= @${option.predicateParameters.length}`;
                option.predicateParameters.push(fromDate);
            }
            ...
            if ($scope.searchInfo.departmentId) {
                option.predicate = option.predicate ? option.predicate + ` && DeptCode == @${option.predicateParameters.length}` : `DeptCode == @${option.predicateParameters.length}`;
                option.predicateParameters.push($scope.searchInfo.departmentId);
            }
        }
