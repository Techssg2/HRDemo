...
ssgApp.controller("pendingTargetPlanController", function (
    ...
    $scope.title = $stateParams.id ? $stateParams.referenceValue : $state.current.name == 'home.pendingTargetPlan.item' ? '' : $stateParams.action.title;
    targetPlans = [];
    $scope.model = {};
    $scope.submitModel = {};
    $scope.warningDialog = {};
    ...
    this.$onInit = async function () {
        ...
        $scope.divisionOptions = {
            ...
            template: function (dataItem) {
                ...
                return `<span class="${dataItem.item.type == 2 || $scope.disableDivison.includes(dataItem.item.id) ? 'k-state-disabled' : ''}">${showCustomDepartmentTitle(dataItem)}</span>`;
            },
            loadOnDemand: false,
            ...
            dataBound: function (e) {
                ...
                for (var i = 0; i < lockTableRows.length; i++) {
                    ...
                    if (!inactive && $scope.visibleSubmitData) {
                        ...
                        row.css("background-color", "#F9F1C7");
                    }
                }
            ...
            while (ite <= to) {
                result.push({
                    ...
                    template: function (dataItem) {
                        ...
                        /*if ($scope.model && $scope.model.divisionId === '')
                            disabledField = true;*/
                        if(disabledField == false){
                            const currentUser = $rootScope.currentUser;
                            if (((currentUser.isStore && currentUser.storePosition == 1) || (!currentUser.isStore && currentUser.hqPosition == 1)) && $scope.model.divisionId == '') {
                                if ($scope.model && $scope.model.divisionId === '')
                                    disabledField = true;
                                var DeptLine = _.find($scope.deptLineList, x => { return x.deptLine.id == $scope.model.deptId }).deptLine;
                                if (((DeptLine.isStore && DeptLine.storePosition == 2) || (!DeptLine.isStore && DeptLine.hqPosition == 2)) && DeptLine.submitSAPCodes.includes(currentUser.sapCode)) {
                                    //User được set làm Submit person của 1 Dept G5 trở lên nếu là HQ hoặc Dept G4 trở lên nếu là Store => Không cần chọn Division => vẫn cho submit
                                    disabledField = false || ($scope.visibleSubmitData && !dataItem.isSubmitted ? false : (dataItem.isSent || !dataItem.isActive || dataItem.isDisable));
                                }
                            }
                        }
    ...
    async function getDivisionTreeByIds(ids) {
        ...
        if (result.isSuccess) {
            ...
            if ((currentUser.isStore && currentUser.storePosition == 1) || (!currentUser.isStore && currentUser.hqPosition == 1)) {
    ...
    async function getSapCodeByDivision(divisionIds, option, allowToGetData = false) {
        if (divisionIds) {
            ...
            if (result.isSuccess) {
                var argsGetTotal = _.cloneDeep(arg);
                argsGetTotal.query.limit = 10000;
                let resultGetTotal = await settingService.getInstance().users.getUsersForTargetPlanByDeptId(argsGetTotal).$promise;
                $scope.total = resultGetTotal.isSuccess ? resultGetTotal.object.count : result.object.count;
    ...
    $scope.save = async function (isSend = false) {
        ...
        data = data.filter(x => !x.isSubmitted);

        if (data.length) {
            let userValidate = _.uniq(data.map(us => us.sapCode));
            ...
            if (!$scope.errors.length) {
                ...
                // dong bo list user from sap
                await updateUserInformationQuoteFromSAP($scope.allValidUsers, (new Date).getFullYear());
                        ...
                        $timeout(function () {
                            if (res.errorCodes && res.errorCodes.length) {
                                ...
                                for (let i = 0; i < res.errorCodes.length; i++) {
                                    ...
                                    } else if (res.errorCodes[i] == 2) { // Code 2: validate shiftcode line 2 valid
                                        $scope.errors.push({ controlName: res.messages[i], message: $translate.instant('INVALID_SHIFT_CODE_TARGET_2') });
                                    } else if (res.errorCodes[i] == 3) { // Code 3: Validate cho TargetPlan Holidays
            ...
            Notification.success($translate.instant('NOT_FOUND_TARGET_PLAN_DETAIL'));
        }
    }
                    ...
                    if (result.errorCodes.length) {
                        ...
                        for (let i = 0; i < result.errorCodes.length; i++) {
                            if (result.errorCodes[i] == 1) {
                                ...
                                $scope.errors.push({ controlName: result.messages[i], message: $translate.instant('TARGET_PLAN_VALIDATE_HOLIDAYS') + `: ${res.object} ` });
                            }
                                ...
                                _.forEach(result.object.data, item => {
                                    ...
                                    else if (item.typeName == commonData.validateTargetPlan.WFH) {
                            ...
                            else if (result.errorCodes[i] == 11) { // Jira 253
                                $scope.errors.push({ controlName: `${result.messages[i]}`, message: `${$translate.instant('TARGET_PLAN_SHIFT_CODE_HQ')}` })
                            } else if (result.errorCodes[i] == 17) {
                                $scope.errors.push({ controlName: `${result.messages[i]}`, message: `${$translate.instant('INACTIVE_SHIFT_CODE_TARGET_1')}` })
                            } else if (result.errorCodes[i] == 18) {
                                $scope.errors.push({ controlName: `${result.messages[i]}`, message: `${$translate.instant('INACTIVE_SHIFT_CODE_TARGET_2')}` })
                            }
                            else {
    ...
    $scope.submit = async function () {
        ...
        /*let total = $('#grid_user_id').data('kendoGrid').dataSource._total;
        $('#grid_user_id').data('kendoGrid').dataSource.pageSize(total);
        setPagingDefault(1, total);
        let divisionIds = !$scope.currentDivisonIds && $rootScope.currentUser ? [$rootScope.currentUser.divisionId] : $scope.currentDivisonIds;
        await getSapCodeByDivision(divisionIds, null, true);
        currentData = await getDetailData();*/

        dataBeforeSubmit = $('#grid_user_id').data('kendoGrid').dataSource._data;

        $scope.model.jsonData = JSON.stringify(dataBeforeSubmit);
        $scope.model.createdById = $rootScope.currentUser.id;
        let saveModel = convertJsonData(true);

        let res = await cbService.getInstance().targetPlan.savePendingBeforeSubmit(saveModel).$promise;
        if (res.isSuccess) {
            let total = $('#grid_user_id').data('kendoGrid').dataSource._total;
            $('#grid_user_id').data('kendoGrid').dataSource.pageSize(total);
            setPagingDefault(1, total);
            ...
            await getSapCodeByDivision(divisionIds, null, true);
            currentData = await getDetailData();
            currentData = currentData.filter(x => !x.isSubmitted);
        }
        ...
        if (!$scope.errors.length) {
            if ($scope.model.divisionId != null && $scope.model.divisionId != '') {
                let divisionDetail = await settingService.getInstance().departments.getDepartmentById({ id: $scope.model.divisionId }, null).$promise;
                $scope.model.divisionName = divisionDetail.object.name;
                $scope.model.divisionCode = divisionDetail.object.code;
            }
            ...
            if (res && res.isSuccess && res.object) {
                // kiem tra lai status
                /*var currentItem = await workflowService.getInstance().common.getItemById({ Id: (res.object.itemId ? res.object.itemId : res.object.id) }, null).$promise
                if (currentItem && currentItem.object.status && currentItem.object.status !== res.object.status) {
                ...
                let error = false;
                let lastItemId = '';
                let lastReferenceValue = '';
                for (let i = 0; i < res.object.length; i++) {
                    let itemId = '';
                    if (res.object[i].id) {
                        itemId = res.object[i].id;
                        let workflowStatusResult = await workflowService.getInstance().workflows.getWorkflowStatus({ itemId: itemId }).$promise;
                        if (workflowStatusResult && workflowStatusResult.object) {
                            let workflowId = workflowStatusResult.object.workflowButtons[0].id;
                            var result = await workflowService.getInstance().workflows.startWorkflow({ workflowId: workflowId, itemId: itemId }, null).$promise;
                    ...
                    if (res.object[i].itemId) {
                        itemId = res.object[i].itemId;
                        var result = await workflowService.getInstance().workflows.startWorkflow({ workflowId: res.object[i].workflowId, itemId: res.object[i].itemId }, null).$promise;
                        if (!result.isSuccess) {
                            error = true;
                        }
                    }
                    lastItemId = itemId;
                    lastReferenceValue = res.object[i].referenceNumber;
                }

                if (!error) {
                    Notification.success($translate.instant('COMMON_WORKFLOW_STARTED'));
                    $state.go('home.targetPlan.item', { id: lastItemId, referenceValue: lastReferenceValue }, { reload: true });
                }
                else {
                    Notification.error($translate.instant('COMMON_WORKFLOW_START_ERROR'));
                }
            }
            else {
                $timeout(function () {
                    if (res.errorCodes && res.errorCodes.length) {
                        ...
                        for (let i = 0; i < res.errorCodes.length; i++) {
                            ...
                            if (res.errorCodes[i] == 3) { // Code 3: Validate cho TargetPlan Holidays 
                            ...
                            else if (res.errorCodes[i] == 12) { //  cac user da co target plan
                                $scope.errors.push({ controlName: `${res.messages[i]}`, message: `${$translate.instant('TARGET_PLAN')}` + ' ' + `${$translate.instant('ALREADY_EXISTS')}` })
                            } else if (res.errorCodes[i] == 17) {
                                $scope.errors.push({ controlName: `${res.messages[i]}`, message: `${$translate.instant('INACTIVE_SHIFT_CODE_TARGET_1')}` })
                            } else if (res.errorCodes[i] == 18) {
                                $scope.errors.push({ controlName: `${res.messages[i]}`, message: `${$translate.instant('INACTIVE_SHIFT_CODE_TARGET_2')}` })
                                ...
                                $scope.errors.push({ controlName: $translate.instant(`SHIFT_PLAN_TARGET_${res.errorCodes[i]}` + " OK "), message: $translate.instant(res.messages[i]) });
                            }
                        }
                    } else {
            ...
            if (x.type == 'Target 1') {
                ...
                if (res.length) {
                    Object.keys(x).forEach(fieldName => {
                        ...
                        if (!haveError && (!x.isSent || !x.isSubmitted) && fieldName.includes('targetField') && (x[fieldName] == null || x[fieldName] == undefined || x[fieldName] === "" || x[fieldName].length == 0) && _.findIndex(res, t => { return t == fieldName }) > -1) {
    ...
    async function checkShowAction() {
        ...
        //Đối với user là Store và < G4 hoặc user là HQ và < G5 thì: Phải chọn đúng division của user thì mới enable
        /*const currentUser = $rootScope.currentUser;
        if (((currentUser.isStore && currentUser.jobGradeValue < findJDG4) || (!currentUser.isStore && currentUser.jobGradeValue < findJDG5)) && $scope.model.divisionId == '') {
            var DeptLine = _.find($scope.deptLineList, x => { return x.deptLine.id == $scope.model.deptId }).deptLine;
            // if (((DeptLine.isStore && DeptLine.jobGradeGrade >= 4) || (!DeptLine.isStore && DeptLine.jobGradeGrade >= 5)) && DeptLine.submitSAPCodes.includes(currentUser.sapCode)) {
            if (((DeptLine.isStore && DeptLine.jobGradeGrade >= findJDG4) || (!DeptLine.isStore && DeptLine.jobGradeGrade >= findJDG5)) && DeptLine.submitSAPCodes.includes(currentUser.sapCode)) {
                //User được set làm Submit person của 1 Dept G5 trở lên nếu là HQ hoặc Dept G4 trở lên nếu là Store => Không cần chọn Division => vẫn cho submit
                /!*$scope.visibleSendData = true;
                $scope.visibleSubmitData = true;*!/
            }
            else {
                $scope.visibleSendData = false;
                $scope.visibleSubmitData = false;
    ...
    async function validateTargetPlanDetail(model) {
        ...
        for (var i = 0; i < model.list.length; i++) {
            await getLeaveBalance(model.list[i].sapCode);
            model.list[i].CFRemain = $scope.CFremain;
        }
    ...
    async function updateUserInformationQuoteFromSAP(allUserValid, year) {
        var result = await ssgexService.getInstance().remoteDatas.updateUserInformationQuoteFromSAP({ sapCodes: allUserValid, year: year }).$promise;
        console.log(result);
    }

    async function getLeaveBalance(sapcode) {
        let userSapCode = sapcode;
        var leaveBalances = 0.0;
        if (userSapCode) {
            ...
            if (res.isSuccess && res.object) {
                for (var i = 0; i < res.object.length; i++) {
                    if (res.object[i].absenceQuotaType == '10') {
                        leaveBalances = res.object[i].remain;
                        break;
                    }
                }
            } else {
