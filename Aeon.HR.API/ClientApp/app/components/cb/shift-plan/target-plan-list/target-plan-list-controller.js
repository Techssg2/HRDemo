...
ssgApp.controller('targetPlanListController', function ($rootScope, $scope, $location, appSetting, localStorageService, $stateParams, $state, moment, Notification, settingService, $timeout, cbService, masterDataService, workflowService, ssgexService, fileService, commonData, $translate, attachmentService, attachmentFile, btaService) {
    ...
    this.$onInit = function () {
            ...
            dataSource: {
                ...
                transport: {
                    read: async function (e) {
                        ...
                        setTimeout(() => {
                            var grid = $("#targetPlanGrid").data("kendoGrid");
                            if (grid && grid.lockedTable) {
                                grid._syncLockedContentHeight(); // ép đồng bộ chiều cao
                            }
                        }, 100);
        ...
        if ($state.current.name === 'home.targetPlan.myRequests') {
            $scope.selectedTab = "1";
        }
        else if ($state.current.name === 'home.targetPlan.allRequests') {
            $scope.selectedTab = "0";
        }
    ...
    function buildArgs(pageIndex, pageSize) {
        ...
        if ($scope.query.userSAPCode) {
            ...
            option.predicate += `TargetPlanDetails.any(x => x.SAPCode == @` + option.predicateParameters.length + `)`;
