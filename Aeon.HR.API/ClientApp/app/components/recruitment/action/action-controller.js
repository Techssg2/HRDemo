...
ssgApp.controller('actionController', function ($rootScope, $scope, $location, appSetting, $stateParams, $state, moment, Notification, $timeout, recruitmentService, commonData, settingService, employeeService, masterDataService, workflowService, ssgexService, fileService, localStorageService, $translate, permissionService) {
    // create a message to display in our view
    var ssg = this;
    $scope.isITHelpdesk = $rootScope.currentUser && $rootScope.currentUser.isITHelpDesk;
    $scope.title = '';
    isItem = true;
    $scope.titleHeader = 'ACTING';
    $scope.title = $stateParams.id ? /*"Acting: " +*/ $stateParams.referenceValue : $state.current.name == 'home.action.item' ? /*"NEW ITEM: ACTING"*/'' : $state.current.name == 'home.action.myRequests' ? "My Acting Requests".toUpperCase() : "All Acting Requests".toUpperCase();

    id = $stateParams.id;
    $rootScope.isParentMenu = false;
    $scope.isDepartmentSAP = false;
    $scope.filter = {};
    $scope.detail = {};
    $scope.errors = [];
    ...
    $('#mySelect').on('change', function () {
        ...
        if (selectedValue === "1") {
            $scope.selectedTab = 1;
            $state.go('home.action.myRequests')

        } else if (selectedValue === "0") {
            $state.go('home.action.allRequests')
        }
    });



    $scope.createItem = async function () {
        $state.go('home.action.item', {}, { reload: true });
    };
    ...
    $scope.departmentOptions = {
        ...
        select: async function (e) {
            ...
            if (dataItem) {
                ...
                $scope.detail.departmentActingPeriodName = dataItem.name;
                ...
                $scope.detail.newJobGradeName = dataItem.jobGradeGrade;
                var intSubGroup = ($scope.detail.newJobGradeName && $scope.detail.newJobGradeName !== '') ? $scope.detail.newJobGradeName : 0;
                if ($scope.detail.newJobGradeName && $scope.detail.newJobGradeName !== '') {
                    if ($scope.detail.newJobGradeName.toString().includes("G")) intSubGroup = $scope.detail.newJobGradeName.replace("G", "");
                }
                ...
                $scope.detail.payScaleArea = intSubGroup > 0 ? "0" + (intSubGroup - 1) : ""
                $scope.detail.employeeSubgroup = "";
                if (intSubGroup > 0) {
                    /*if (intSubGroup > 4) {
                        intSubGroup = intSubGroup - 1;
                    }*/
                    $scope.detail.employeeSubgroup = "0" + (intSubGroup - 1);
                }

                // Neu la G5 thi lay theo Jobgrade User - 1
                if ($scope.detail.jobGradeValue && $scope.detail.jobGradeValue == 5) {
                    $scope.detail.employeeSubgroup = "0" + ($scope.detail.jobGradeValue - 1);
                }
    ...
    function filterDepartmentHasNoSAPCode(departmentInfo) {
        ...
        try {
            ...
            if (!$.isEmptyObject(departmentInfo)) {
        ...
        change: async function (e) {
            if (e.sender.text()) {
                ...
                if (result.isSuccess) {
                    ...
                    if (result.object.jobGradeValue >= 6) //job Grade tu G5+ tro len
                        $scope.jobGradeNewCaption = "G" + (result.object.jobGradeValue);
                    else
                        $scope.jobGradeNewCaption = "G" + (result.object.jobGradeValue + 1);
                    ...
                    $timeout(function () {
                        ...
                        $scope.detail.jobGradeId = result.object.jobGradeId;
    ...
    $scope.jobTitlesDataSource = {
            ...
            transport: {
                read: async function (e) {
                    if ($scope.jobGradeCaption) {
                        ...
                        //queryArgs.predicate = queryArgs.predicate + ' and ' + '(Code.contains(@2))';
                        //queryArgs.predicateParameters.push('ACT');
        ...
        change: async function (e) {
            ...
            $scope.detail.positionName = 'Acting ' + e.sender.text();
        }
    };
    ...
    async function getActingByReferenceNumber(referenceNumber) {
        ...
        if (result.isSuccess) {
            ...
            $scope.statusTranslate = $rootScope.getStatusTranslate(result.object.data.acting.status);
            $scope.actingStatus = result.object.data.acting.status;
            $scope.actingPositionId = result.object.data.acting.positionId;
            $scope.jobGradeCaption = result.object.data.acting.jobGradeName;
            $scope.jobGradeOldCaption = result.object.data.acting.jobGradeName;
            $scope.jobGradeNewCaption = "G" + (result.object.data.acting.jobGradeValue + 1);
            //không dùng read dropdown được do ko thể load data lên được khi get detail
            // var positionSelect = $("#titleInActingPeriodCode").data("kendoDropDownList");
            // positionSelect.dataSource.read();
            await getPositions();
            ...
            $timeout(async function () {
                ...
                $scope.detail.formatCreated = formatDate($scope.detail.created);
                ...
                if ($stateParams.id) {
                    ...
                    //lay department code cho hover cua department in acting period
                    let deptdetail = await settingService.getInstance().departments.getDepartmentById({ id: $scope.detail.departmentId }, null).$promise;
                    $scope.detail.newDeptOrLineCode = deptdetail.object.code;
                }
    ...
    $scope.goalGridOptions = {
        ...
        columns: [{
            ...
            width: "250px",
            template: function (dataItem) {
                return `<input class="k-textbox w100" ng-change="onChangeGoal(dataItem)" type="text" style="width:100%" ng-model="dataItem.goal"/>`
        ...
        },
        {
            ...
            width: "130px",
            template: function (dataItem) {
                return `<input id="weightId" kendo-numeric-text-box class="w100" style="width:100%" k-ng-model="dataItem.weight"/>`
            }
        },
        {
            field: "",
            ...
            template: function (dataItem) {
                if (!dataItem.selected) {
                    if (!$scope.disable)
                        return `<a class="btn-border-upgrade btn-delete-upgrade" ng-click="deleteRow('goal', dataItem)"></a>`;
                    else
                        return '';
                }
            }
    ...
    $scope.appraisingDetailGridOptions = function (dataItem) {
        return {
            ...
            },
            {
                ...
                width: "180px",
                template: function (dataItem) {
                    return `<input class="k-textbox w100" type="text" style="width:100%" ng-model="dataItem.target"/>`
                }
            },
            {
                field: "actual",
    ...
    $scope.compulsoryGridOptions = {
        ...
        columns: [{
            ...
            width: "180px",
            template: function (dataItem) {
                return `<input class="k-textbox w100" type="text" style="width:100%" ng-model="dataItem.courseName"/>`
            }
        },
        {
            field: "duration",
            ...
            width: "100px",
            template: function (dataItem) {
                return `<input class="k-textbox w100" type="text" style="width:100%" ng-model="dataItem.duration"/>`
            }
        }, {
            field: "actualResult",
            title: 'Actual Result',
            width: "100px",
            template: function (dataItem) {
                return `<input class="k-textbox w100 readonly input-default" disabled style="width:100%" type="text" ng-model="dataItem.actualResult"/>`
            }
        },
        {
            field: "",
            ...
            template: function (dataItem) {
                if (!dataItem.selected) {
                    return `<a class="btn-border-upgrade btn-delete-upgrade" ng-click="deleteRow('compulsoryTraining', dataItem)"></a>`;
                }
            }
        }
        ]
    ...
    $scope.buildQuery = function () {
        ...
        if ($scope.filter.departmentId) {
            ...
            option.predicate = (option.predicate ? option.predicate + ' and ' : option.predicate) + `(DeptCode =@${option.predicateParameters.length})`;
            option.predicateParameters.push($scope.filter.departmentId);
    ...
    $scope.ngOnit = async function () {
        if ($state.current.name === 'home.action.myRequests') {
            $scope.selectedTab = "1";
        }
        else if ($state.current.name === 'home.action.allRequests') {
            $scope.selectedTab = "0";
        }
        ...
        if ($state.current.name == commonData.stateActing.myRequests || $state.current.name == commonData.stateActing.allRequests) {
            ...
            $scope.detail.createdByFullName = $rootScope.currentUser.fullName;
            ...
            if ($stateParams.referenceValue) {
                ...
                var permResult = await permissionService.getInstance().permission.getPerm({ itemId: $stateParams.id }).$promise;
                //Permission
                if ((2 & permResult.object) == 2) {
                    $scope.disable = false;
                } else {
                    $scope.disable = true;
                }
    ...
    async function getPositions() {
        if ($scope.jobGradeCaption) {
            ...
            queryArgs.predicateParameters.push('Position');
            //queryArgs.predicateParameters.push($scope.jobGradeCaption);
            queryArgs.predicateParameters.push($scope.jobGradeNewCaption ? $scope.jobGradeNewCaption : $scope.jobGradeCaption);
            // Xu ly cac phieu cu
            if ($scope.jobGradeOldCaption) {
                queryArgs.predicate = queryArgs.predicate + ' or ' + '(JobGrade.Caption = @2)';
                queryArgs.predicateParameters.push($scope.jobGradeOldCaption);
                //bổ sung để view các Position cũ
                queryArgs.predicate = queryArgs.predicate + ' or ' + '(Id = @3)';
                queryArgs.predicateParameters.push($scope.actingPositionId);
            }
            //queryArgs.predicate = queryArgs.predicate + ' and ' + '(Code.contains(@2))';
            //queryArgs.predicateParameters.push('ACT');
            queryArgs.limit = 1000;
            ...
            if (result && result.isSuccess) {
                var positionSelect = $("#titleInActingPeriodCode").data("kendoDropDownList");
                let dataSource = result.object.data;
                dataSource = _.filter(dataSource, x => {
                    //if ($scope.actingStatus && ($scope.actingStatus == 'Draft' || $scope.actingStatus == 'Requested To Change')) {
                    //    return x.jobGradeCaption == $scope.jobGradeNewCaption;
                    //} else {
                    //    return x.id == $scope.actingPositionId || x.jobGradeCaption == $scope.jobGradeNewCaption;
                    //}
                    return x.id == $scope.actingPositionId || x.jobGradeCaption == $scope.jobGradeNewCaption;
                });
                positionSelect.setDataSource(dataSource);
            }
        }
    }
