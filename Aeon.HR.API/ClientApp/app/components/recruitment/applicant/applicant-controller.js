...
ssgApp.controller('applicantController', function ($rootScope, $scope, $location,$translate, appSetting, $stateParams, $state, moment, commonData, recruitmentService, masterDataService, settingService, Notification, $timeout, fileService, localStorageService) {
    ...
    $('#mySelect').on('change', function () {
        ...
        if (selectedValue === "1") {
            $scope.selectedTab = 1;
            $state.go('home.applicant.myRequests')

        } else if (selectedValue === "0") {
            $scope.selectedTab = 0;
            $state.go('home.applicant.allRequests')
        }
    });
    ...
    this.$onInit = async () => {
        if ($state.current.name === 'home.applicant.myRequests') {
            $scope.selectedTab = "1";
        }
        else if ($state.current.name === 'home.applicant.allRequests') {
            $scope.selectedTab = "0";
        }
    ...
    $scope.applyHtml = async function () {
        let fromDateElement = document.getElementById("fromDate_id");
        let toDateElement = document.getElementById("toDate_id");

        let wrappedFromDate = angular.element(fromDateElement);
        let wrappedToDate = angular.element(toDateElement);

        wrappedFromDate.addClass("form-control");
        wrappedToDate.addClass("form-control");
