...
app.constant("commonData", {
    ...
    requestToHires: {
        ...
        trackingImport: 'home.requestToHire.trackingImport'
    },
    reasonType: {
...
function isRestrictedPropertiesEmpty(fields) {
    ...
    if (fields) {
        for (var i = 0; i < fields.length; i++) {
            ...
            if (field.isRequired) {
                var isChecked = false;
                var fieldPattern = field.fieldPattern;
                ...
                var eles = $("input[k-ng-model], input[ng-model], select[k-ng-model], select[ng-model]").filter(function () {
                    var kNgModel = $(this).attr('k-ng-model');
                    var ngModel = $(this).attr('ng-model');
                    return (kNgModel && kNgModel.substring(kNgModel.length - fieldPattern.length) === fieldPattern) ||
                        (ngModel && ngModel.substring(ngModel.length - fieldPattern.length) === fieldPattern);
                });
...
function disableElement(disabled) {
    if (disabled) {
        $("button:contains('Add more')").hide();
        $("a.btn-delete-upgrade").hide();
        $("button:contains('Thêm mới')").hide();
        $("input:not(.input-default),select:not(.input-default),textarea:not(.input-default),button:contains('Add more')").attr("disabled", "disabled");
        $("input:not(.input-default),select:not(.input-default),textarea:not(.input-default),button:contains('Thêm mới')").attr("disabled", "disabled");
        $("a:contains('Delete')").filter(function () {
            return $(this).text().trim() === "Delete";
        }).hide();
    } else {
        $("button:contains('Add more')").show();
        $("a.btn-delete-upgrade").show();
        $("button:contains('Thêm mới')").show();
        $("input:not(.input-default),select:not(.input-default),textarea:not(.input-default),button:contains('Add more')").removeAttr("disabled");
        $("input:not(.input-default),select:not(.input-default),textarea:not(.input-default),button:contains('Thêm mới')").removeAttr("disabled");
        $("a:contains('Delete')").filter(function () {
            return $(this).text().trim() === "Delete";
        }).show();
    }
