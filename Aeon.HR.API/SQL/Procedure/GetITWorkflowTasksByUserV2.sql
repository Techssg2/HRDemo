USE [AEONHRPRODDB]
GO
/****** Object:  StoredProcedure [dbo].[GetITWorkflowTasksByUser]    Script Date: 8/4/2025 10:31:01 AM ******/
SET ANSI_NULLS ON
GO
...
FROM (
         ...
         SELECT
			...
			case 
				...
				WHEN itt.ModuleCode = 'Edoc1' then
				case
					when wfTa.ReferenceNumber like 'AC%' then (SELECT pr.FullUrl FROM [AEON.PRODUCT.Edocument].[dbo].[AccrualRequests] pr WHERE pr.Id = wfTa.ItemId)
				end
				else 
					null 
				end
            ...
            CASE
				...
				WHEN itt.ModuleCode = 'Edoc1' then
				case
					when wfTa.ReferenceNumber like 'AC%' then 'Accrual Requests'
				end
         ...
         WHERE
             wfTa.IsCompleted = 0
         --AND wfTa.AssignedToDepartmentId IS NOT NULL --1/8 khoa
           ...
           AND (
            ...
            OR mdg.GroupCode IS NOT NULL
             ...
             )OR wfTa.AssignedToId = @UserId)--1/8 bổ sung trường hợp assign 1 user cụ thể
