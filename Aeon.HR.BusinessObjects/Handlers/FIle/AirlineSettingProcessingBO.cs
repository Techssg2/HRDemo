...
{
    public class AirlineSettingProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "AirlineSetting";
        public AirlineSettingProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var airlines = await _uow.GetRepository<Airline>().FindByAsync<AirlineViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (airlines.Any())
            {
                for (int rowNum = 0; rowNum < airlines.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = airlines.ElementAt(rowNum);
