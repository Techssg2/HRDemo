...
{
    public class ApplicantStatusListRecruitmentProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "ApplicantStatusListRecruitment";
        public ApplicantStatusListRecruitmentProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var applicantStatusLists = await _uow.GetRepository<ApplicantStatusRecruitment>().FindByAsync<ApplicantStatusRecruitmentViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (applicantStatusLists.Any())
            {
                for (int rowNum = 0; rowNum < applicantStatusLists.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = applicantStatusLists.ElementAt(rowNum);
