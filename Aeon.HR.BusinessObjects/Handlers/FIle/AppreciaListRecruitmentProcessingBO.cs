...
{
    public class AppreciaListRecruitmentProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "AppreciationListRecruitment";
        public AppreciaListRecruitmentProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var appreciationLists = await _uow.GetRepository<AppreciationListRecruitment>().FindByAsync<AppreciationListRecruitmentViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (appreciationLists.Any())
            {
                for (int rowNum = 0; rowNum < appreciationLists.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = appreciationLists.ElementAt(rowNum);
