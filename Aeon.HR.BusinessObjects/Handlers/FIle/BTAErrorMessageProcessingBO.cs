...
{
    public class BTAErrorMessageProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "BTAErrorMessage";
        public BTAErrorMessageProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var btaErrorMessages = await _uow.GetRepository<BTAErrorMessage>().FindByAsync<BTAErrorMessageViewModel>(parameters.Order, parameters.Page, parameters.Limit, parameters.Predicate, parameters.PredicateParameters);
            if (btaErrorMessages.Any())
            {
                for (int rowNum = 0; rowNum < btaErrorMessages.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = btaErrorMessages.ElementAt(rowNum);
