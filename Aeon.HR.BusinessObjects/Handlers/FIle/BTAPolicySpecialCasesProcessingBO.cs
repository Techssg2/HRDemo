...
{
    public class BTAPolicySpecialCasesProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "BTAPolicySpecialCases";
        public BTAPolicySpecialCasesProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var items = await _uow.GetRepository<BTAPolicySpecial>().FindByAsync<BTAPolicySpecialViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (items.Any())
            {
                for (int rowNum = 0; rowNum < items.Count(); rowNum++)
                {
                    ...
                    for (int j = 0; j < fieldMappings.Count; j++)
                    {
						...
						if (fieldMapping.Name == "UserDepartmentName")
						{
                            var deptName = data.Department.Name;
                            HandleCommonType(row, deptName, j, fieldMapping);
                            continue;
                        }
                        if (fieldMapping.Name == "UserJobGradeCaption")
                        {
                            var jobGrade = data.JobGrade.Caption;
                            HandleCommonType(row, jobGrade, j, fieldMapping);
                            continue;
                        }
