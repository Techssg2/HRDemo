...
{
    public class BusinessModelProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "BusinessModel";
        public BusinessModelProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var jobGrades = await _uow.GetRepository<BusinessModel>().FindByAsync<BusinessModelViewModel>(parameters.Predicate, parameters.PredicateParameters, "Created");
            if (jobGrades.Any())
            {
