...
{
    public class BusinessModelProcessingUnitMappingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "BusinessModelUnitMapping";
        public BusinessModelProcessingUnitMappingBO(IUnitOfWork uow) : base(uow)
        {}
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var jobGrades = await _uow.GetRepository<BusinessModelUnitMapping>().FindByAsync<BusinessModelUnitMappingViewModel>(parameters.Predicate, parameters.PredicateParameters, "Created");
            if (jobGrades.Any())
            {
                for (int rowNum = 0; rowNum < jobGrades.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = jobGrades.ElementAt(rowNum);
