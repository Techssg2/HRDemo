...
{
    public class FlightNumberSettingProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "FlightNumberSetting";
        public FlightNumberSettingProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var flightNumbers = await _uow.GetRepository<FlightNumber>().FindByAsync<FlightNumberViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (flightNumbers.Any())
            {
                for (int rowNum = 0; rowNum < flightNumbers.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = flightNumbers.ElementAt(rowNum);
