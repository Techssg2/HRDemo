...
{
    public class HeadcountProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "Headcount";
        public HeadcountProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var headcounts = await _uow.GetRepository<HeadCount>().FindByAsync<ItemListHeadCountViewModel>(parameters.Predicate, parameters.PredicateParameters);
            if (headcounts.Any())
            {
                headcounts = headcounts.OrderBy(x => x.DepartmentName).ThenByDescending(y => y.JobGradeValue);
