...
{
    public class HolidayScheduleProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "HolidaySchedule";
        public HolidayScheduleProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var holidaySchedules = await _uow.GetRepository<HolidaySchedule>().FindByAsync<HolidayScheduleViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (holidaySchedules.Any())
            {
                for (int rowNum = 0; rowNum < holidaySchedules.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = holidaySchedules.ElementAt(rowNum);
