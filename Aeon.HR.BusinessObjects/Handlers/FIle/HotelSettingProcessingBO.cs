...
{
    public class HotelSettingProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "HotelSetting";
        public HotelSettingProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var hotels = await _uow.GetRepository<Hotel>().FindByAsync<HotelViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (hotels.Any())
            {
                for (int rowNum = 0; rowNum < hotels.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = hotels.ElementAt(rowNum);
