...
{
    public class JobGradeProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "JobGrade";
        public JobGradeProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var jobGrades = await _uow.GetRepository<JobGrade>().FindByAsync<JobGradeViewModel>(parameters.Predicate, parameters.PredicateParameters, "Grade");
            if (jobGrades.Any())
            {
