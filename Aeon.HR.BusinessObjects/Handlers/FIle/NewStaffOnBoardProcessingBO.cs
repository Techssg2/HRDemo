...
{
    public class NewStaffOnBoardProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "NewStaffOnBoard";
        public NewStaffOnBoardProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            if (records.Any())
            {
                ...
                var exportNewStaffOnBoardViewModels = new List<ExportApplicant>();
                foreach (var _record in records)
                {
                    var record = Mapper.Map<ApplicantViewModel>(_record);
                    exportNewStaffOnBoardViewModels.Add(new ExportApplicant
                    {
                ...
                for (int rowNum = 0; rowNum < exportNewStaffOnBoardViewModels.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = exportNewStaffOnBoardViewModels.ElementAt(rowNum);
