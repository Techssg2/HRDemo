...
{
    public class PositionListRecruitmentProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "PositionListRecruitment";
        public PositionListRecruitmentProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var positionLists = await _uow.GetRepository<MasterData>().FindByAsync<MasterDataViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (positionLists.Any())
            {
                for (int rowNum = 0; rowNum < positionLists.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = positionLists.ElementAt(rowNum);
                    for (int j = 0; j < fieldMappings.Count; j++)
                    {
                        ...
                        //if(fieldMapping.Name == "JobGradeCaption")
                        //{
                        //    if(value == null)
                        //    {
                        //        value = "";
