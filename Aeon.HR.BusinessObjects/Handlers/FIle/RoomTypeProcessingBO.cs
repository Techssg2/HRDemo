...
{
    public class RoomTypeProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "RoomTypeSetting";
        public RoomTypeProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var roomTypes = await _uow.GetRepository<RoomType>().FindByAsync<RoomTypeViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (roomTypes.Any())
            {
                for (int rowNum = 0; rowNum < roomTypes.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = roomTypes.ElementAt(rowNum);
