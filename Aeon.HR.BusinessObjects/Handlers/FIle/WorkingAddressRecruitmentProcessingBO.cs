...
{
    public class WorkingAddressRecruitmentProcessingBO : BaseExcelProcessing, IExcelProcessingBO
    {
        protected override string Json => "export-mapping.json";
        protected override string JsonGroupName => "WorkingAddressRecruitment";
        public WorkingAddressRecruitmentProcessingBO(IUnitOfWork uow) : base(uow)
        {
        ...
        public async Task<ResultDTO> ExportAsync(QueryArgs parameters)
        {
            ...
            // Add Row
            var workingAddress = await _uow.GetRepository<WorkingAddressRecruitment>().FindByAsync<WorkingAddressRecruitmentViewModel>(parameters.Order, 1, Int32.MaxValue, parameters.Predicate, parameters.PredicateParameters);
            if (workingAddress.Any())
            {
                for (int rowNum = 0; rowNum < workingAddress.Count(); rowNum++)
                {
                    DataRow row = tbl.Rows.Add();
                    var data = workingAddress.ElementAt(rowNum);
