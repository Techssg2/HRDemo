angular
  .module("ssg.module.academy.training-report.my-item.ctrl", [
    "kendo.directives",
  ])
  .controller(
    "TrainingReportMyItemController",
    function (
      $rootScope,
      $scope,
      $stateParams,
      ...
      $scope.title = $translate.instant("MY_TRAINING_REPORT");
      $scope.currentQuery = {
      ...
      async function getTrainingReportList(option) {
        ...
        if (option) {
          let result = await TrainingReportService.listMyReport(
            $scope.currentQuery
          ).$promise;
