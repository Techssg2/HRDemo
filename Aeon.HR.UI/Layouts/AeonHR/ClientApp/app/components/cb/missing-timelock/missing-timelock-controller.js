...
ssgApp.controller("missingTimelockController", function (
    ...
    $scope.title = $stateParams.id ? $translate.instant('MISSING_TIMECLOCK_EDIT_TITLE') + $stateParams.referenceValue : $state.current.name == 'home.missingTimelock.item' ? $translate.instant('MISSING_TIMECLOCK_NEW_TITLE') : $state.current.name == 'home.missingTimelock.myRequests' ? $translate.instant('MISSING_TIMECLOCK_MY_REQUETS') : $translate.instant('MISSING_TIMECLOCK_ALL_REQUETS');
    //edit / new
    ...
    $scope.reasonDetailGridOptions = {
        ...
        },
        {
            ...
            template: function (dataItem) {
                if (dataItem.typeActualTime) {
                    ...
                    return `<input readonly class="k-input w100" type="text" ng-model="dataItem.shiftCode" />`;
                }
        ...
        },
        {
            ...
            template: function (dataItem) {
                return `<input name="others" ng-readonly="!isOther(dataItem)" class="k-input" type="text" ng-model="dataItem.others" style="width: 100%"/>`;
            }
        },
        {
            //title: "Actions",
            ...
            template: function (dataItem) {
                return `<a class="btn btn-sm default red-stripe" ng-if="!statusCompleted" ng-click="deleteRecord(dataItem.id)">{{'COMMON_BUTTON_DELETE'|translate}}</a>`;
            }
        }
        ],
        selectable: true,
