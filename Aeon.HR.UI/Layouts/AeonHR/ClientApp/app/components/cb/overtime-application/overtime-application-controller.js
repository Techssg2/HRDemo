...
ssgApp.controller('overtimeApplicationController',
    function (
        ...
        $scope.title = $stateParams.id ? $translate.instant('OVERTIME_TIMECLOCK_EDIT_TITLE') + $stateParams.referenceValue : $state.current.name == 'home.overtimeApplication.item' ? $translate.instant('OVERTIME_APPLICATION_NEW_TITLE') : $state.current.name == 'home.overtimeApplication.myRequests' ? $translate.instant('OVERTIME_APPLICATION_MY_REQUETS') : $translate.instant('OVERTIME_APPLICATION_ALL_REQUETS');
            ...
            if (result.isSuccess && result.object && result.object.data.length) {
                ...
                $scope.form.overtimeList = $scope.model.overtimeList.map(x => {
                    return {
                        ...
                        dateOfOT: moment(x.date).format('DD/MM/YYYY'),
                        OTProposalHour: mapOTHour(x.proposalHoursFrom, x.proposalHoursTo),
        ...
        function mapOTHour(from, to) {
            ...
            if (isStoreCurrentUser == false && result >= 5) {
            ...
            if (result.isSuccess) {
                $timeout(async function () {
                    ...
                    if ($stateParams.type === commonData.stateOvertime.item) {
                        ...
                        result.object.overtimeItems.forEach(item => {
                            ...
                            item['dateOfOT'] = moment(item.date).format('DD/MM/YYYY');
                            ...
                            item['OTProposalHour'] = mapOTHour(item.proposalHoursFrom, item.proposalHoursTo);
                            item['actualOTHour'] = item.actualHoursFrom && item.actualHoursTo ? mapOTHour(item.actualHoursFrom, item.actualHoursTo) : '';
                        ...
                        $scope.form.overtimeList.forEach(element => {
                            ...
                            if (element.fromActual && element.toActual) {
                                element.actualOTHour = mapOTHour(element.fromActual, element.toActual);
                            }
                        });
                        $scope.warning = $translate.instant('OVERTIME_APPLICATION_WARNING') + ' ' + $scope.dateOfOT.join(', ');
                ...
                if ($stateParams.type === commonData.stateOvertime.approve) {
                    result.object.overtimeItems.forEach(item => {
                        ...
                        item['OTProposalHour'] = mapOTHour(item.proposalHoursFrom, item.proposalHoursTo);
                        item['actualOTHour'] = mapOTHour(item.actualHoursFrom, item.actualHoursTo);
                    });
        ...
        async function getDepartmentByReferenceNumber(referenceNumber, id) {
            ...
            if (res.isSuccess) {
                setDropDownTree('deptDivision', res.object.data);
            }
        }
                    ...
                    action: function (e) {
                        ...
                        $scope.userGridReview.forEach(item => {
                            let value = {
                                ...
                                reasonOfOT: '',
                                OTProposalHour: caculateOTHourUserReview($scope.userOTReview.from, $scope.userOTReview.to),
