...
ssgApp.controller("shiftExchangeApplicationController", function (
    ...
    function buildArgs(pageIndex, pageSize) {
        ...
        if ($scope.query.toDate) {
            option.predicate = (option.predicate ? option.predicate + ' and ' : option.predicate) + `Created < @${option.predicateParameters.length}`;
            option.predicateParameters.push(moment($scope.query.toDate).add(1, 'days').format('YYYY-MM-DD'));
        }
    ...
    $scope.onChangeDivision = async function (value) {
        ...
        if ($scope.form.exchangingShiftItems.length > 0 && $scope.perm && !value) {
            ...
            if (!value) { 
                ...
                //await getUserCheckedHeadCount($rootScope.currentUser.divisionId);
            } else {
                //await getSapCodeByDivison(1, $scope.limitDefaultGrid, '');
            }
    ...
    $scope.shiftExchangeEditListGridOptions = {
        ...
        },
        {
            ...
            template: function (dataItem) {
                return `<input name="others" ng-readonly="true" class="k-input" type="text" ng-model="dataItem.employeeCode" style="width: 100%"/>`;
            }
        },
        {
            field: "fullName",
            ...
            // }
            template: function (dataItem) {
                return `<input name="others" ng-readonly="true" class="k-input" type="text" ng-model="dataItem.fullName" style="width: 100%"/>`;
            }
        },
        {
            field: "quotaERD",
            ...
            // }
            template: function (dataItem) {
                return `<input name="others" ng-readonly="true" class="k-input" type="text" ng-model="dataItem.quotaERD" style="width: 100%"/>`;
            }
        },
        // {
        //     field: "quotaDOFL",
        ...
        },
        {
            ...
            width: "200px",
            template: function (dataItem) {
                return `<input name="otherReason" ng-readonly="!isOther(dataItem)" class="k-input" type="text" ng-model="dataItem.otherReason" style="width: 100%"/>`;
            }
        },
        {
            field: "isERD",
            ...
            template: function (dataItem) {
                ...
                id="isERD{{dataItem.no}}" class="k-checkbox" style="width: 100%;" />
                <label class="k-checkbox-label cbox" for="isERD{{dataItem.no}}"></label>`
            }
        },
        {
            title: "Actions",
            width: "100px",
            template: function (dataItem) {
                return `<a class="btn btn-sm default red-stripe" ng-click="deleteRecord(dataItem.no)">{{'COMMON_BUTTON_DELETE'|translate}}</a>`;
            }
        }
        ]
    };
    ...
    function validationDuplicateValue(items) {
        ...
        items.forEach(item => {
            if (item.exchangeDate) {
                let duplicates = itemsClone.filter(x => x.employeeCode === item.employeeCode && moment(x.exchangeDate.toDateString().slice(0, 10)).isSame(item.exchangeDate.toDateString().slice(0, 10)) && x.newShiftCode == item.newShiftCode);
                itemsClone = itemsClone.filter(x => !(x.employeeCode === item.employeeCode && moment(x.exchangeDate.toDateString().slice(0, 10)).isSame(item.exchangeDate.toDateString().slice(0, 10))));
            ...
            <div class="col-md-4" style="margin-top: -25px;">
                <div class="form-group">
                    ...
                    <div class="col-md-7" style="margin-left: -10px;">
                        <span style="color: #b42e83 !important;font-family: Nova-Regular !important;font-size: 18px !important;font-weight: 700; margin: 10px 0;margin: 0;=line-height: 1.42857143;=text-transform: uppercase !important;">${$translate.instant('COMMON_BUTTON_REVIEWUSER')}</span>
                    </div>
                </div>
            </div>
        </div>
