...
ssgApp.controller("targetPlanController", function (
            ...
            while (ite <= to) {
                result.push({
                    ...
                    template: function (dataItem) {
                        ...
                        input_html = `<input ng-change="onChange(dataItem)" style="border: 1px solid #c9c9c9; text-align: center" class="k-input w100" ng-model="dataItem.targetField${moment(temp).format('YYYYMMDD')}" ng-readonly="${(disabledField || !allowEdit)}"/>`;
    ...
    $scope.appove = async function () {
        ...
        $scope.save(isSend);
        deferred.resolve({ object: $scope.model, isSuccess: true });
