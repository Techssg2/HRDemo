...
ssgApp.controller('handoverController', function ($rootScope, $scope, $location, appSetting, $stateParams, $state, Notification, commonData, recruitmentService, masterDataService, settingService, $timeout, ssgexService, fileService) {
    ...
    // $scope.title = '';
    isItem = true;
    $scope.title = $stateParams.id ? "HANDOVER" + $stateParams.referenceValue : $state.current.name == 'home.handover.item' ? "New item: Handover" : $stateParams.action.title;
        ...
        $scope.handoverGridOptions = {
            ...
            },
            {
                ...
                template: function (dataItem) {
                    ...
                    id="isCancel{{dataItem.no}}" class="k-checkbox" style="width: 100%;" disabled="disabled"/>
        ...
        },
        {
            ...
            template: function (dataItem) {
                if (!$stateParams.id) {
                    ...
                    return `<input kendo-numeric-text-box id="quantity" k-min="0" name="quantity" k-format="'#,0'" k-ng-model="dataItem.quantity" style="width: 100%;background-color: #eee;" readonly/>`
                }
            }
        }, {
            field: "notes",
