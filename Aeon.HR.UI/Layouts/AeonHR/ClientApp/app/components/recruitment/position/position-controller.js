        ...
        },
        {
            ...
            template: function (dataItem) {
                ...
                if (dataItem.status != commonData.positionStatus.Closed) {
                    return `<a class='btn btn-sm default blue-stripe' ng-show="isVisibleRe_AssignBtn" ng-click="actionPosition('reassign',dataItem)">Re-Assign</a>
                    <a role='button'  ng-click="actionPosition('close',dataItem)"  class='btn btn-sm default red-stripe' >Close</a>`
                } else if (dataItem.status == commonData.positionStatus.Closed) {
                    return `<a role='button'  ng-click="actionPosition('open',dataItem)"  class='btn btn-sm default green-stripe' >Open</a>`
                } else {
                    return '';
                }
            }
                ...
                return `
                            <div style="display: flex; flex-wrap: wrap;">
                                <div ng-repeat="position in dataItem" style="flex: 0 0 20%; margin-bottom: 20px; display: flex; justify-content: center;">
                                    <div class="k-card" style="width: 96%;">
                                        <div class="k-card-body text-right" style="background-color: #D3409E;">
                                            <h5 class="k-card-title" style="color: white; margin-bottom: 0px">{{position.deptDivisionName}}</h5>
                                            <p class="mrb10" style="color: white;">{{position.locationName}}</p>
                                            <a style="color: white; text-decoration: underline; display: block;" ui-sref="home.position.item({ id: position.id })" ui-sref-active="active">Applicant ({{position.applicantsCount}})</a>
                                            <a style="color: white; text-decoration: underline; display: block" ui-sref="home.position.item({ id: position.id })" ui-sref-active="active">Hired ({{position.hiredApplicantsCount}})</a>
                                            <p style="color: white;">Required ({{position.quantity}})</p>
                                        </div>
                                        <div class="k-card-actions" style="background-color: #911D62; display: flex; justify-content: space-between; align-items: center;">
            ...
            template: function (dataItem) {
                if (dataItem.id) {
                    if (dataItem.selected) {
                        // return `<a kendo-button role='button' type='submit' ng-click="excuteAction('save',dataItem, $event)"  class="btn btn-sm default green-stripe " >Save</a>` +
                        //     `<a kendo-button role='button'  ng-click="excuteAction('cancel',dataItem)"   class="btn btn-sm default " >Cancel</a>`

                        return `<a class="btn btn-sm default green-stripe " ng-click="excuteAction('save',dataItem, $event)">Save</a> 
                            <a class="btn btn-sm default " ng-click="excuteAction('cancel',dataItem)">Cancel</a>`
                    } else {
                        return `<a ng-if="form.status != 2" ng-click="excuteAction('edit',dataItem)" class='btn btn-sm default blue-stripe'>Edit</a>`;
                    }
                }
            }
        }],
