var ssgApp = angular.module("ssg.settingRecruitmentApplicantStatusModule", [
    "kendo.directives"
]);
ssgApp.controller("settingRecruitmentApplicantStatusController", function (
    $rootScope,
    $scope,
    $location,
    $stateParams,
    ...
    $scope.total = 0;
    $scope.keyword = '';
    $scope.dataApplicantStatus = [];
    //API
    async function getAllApplicantStatus(option) {
        ...
        var result = await settingService.getInstance().recruitment.getApplicantStatus($scope.currentQuery).$promise;
        if (result.isSuccess) {
            $scope.dataApplicantStatus = result.object.data;
            var count = (($scope.currentQuery.page - 1) * $scope.currentQuery.limit) + 1;
            $scope.dataApplicantStatus.forEach(x => {
                x["select"] = false;
                x["no"] = count;
                count = count + 1;
            });
            ...
            $scope.dataApplicantStatus.push(nValue);
            $scope.total = result.object.count;
            if (option) {
                option.success($scope.dataApplicantStatus);
            }
            else {
    ...
    async function findApplicantStatusByCode(code, id) {
        let queryArgs = {
            predicate: '',
            predicateParameters: [],
        ...
        var result = await settingService.getInstance().recruitment.getApplicantStatusRecruitmentByCode(queryArgs).$promise;
    ...
    $scope.applicantStatusOptions = {
        dataSource: {
            ...
            transport: {
                read: async function (e) {
                    await getAllApplicantStatus(e);
                }
            },
            schema: {
                total: () => { return $scope.total },
                data: () => { return $scope.dataApplicantStatus }
            }
        },
        sortable: false,
        editable: {
        ...
        columns: [
            ...
            },
            {
                ...
                template: function (dataItem) {
                    if (dataItem.select) {
                        // return `<input class="k-textbox w100" name="arrangement" ng-model="dataItem.arrangement"/>`;
                        return `
                        <input kendo-numeric-text-box ng-model="dataItem.arrangement" k-options="numeric" style="width: 100%;" />
                        `
                    } else {
                        return `<span>{{dataItem.arrangement}}</span>`;
                    }
                }
            },
            {
    ...
    async function create(model) {
        ...
        if (editExist) {
            ...
            if (errors.length > 0) {
                ...
                var checkCode = await findApplicantStatusByCode(model.code);
                if (checkCode) {
                    Notification.error("Code " + model.code + " has existed in Applicant Status");
                }
                else{
                    settingService.getInstance().recruitment.addApplicantStatus({ Id: model.id, Code: model.code, Name: model.name, Arrangement: model.arrangement }).$promise.then(function (result) {
                        if (result.isSuccess) {
    ...
    function checkEdit() {
        var result = false;
        $scope.dataApplicantStatus.forEach(item => {
    ...
    async function save(model) {
        ...
        if (errors.length > 0) {
            ...
            var checkCode = await findApplicantStatusByCode(model.code, model.id);
            if (checkCode) {
                Notification.error("Code " + model.code + " has existed in Applicant Status");
            }
            else {
                settingService.getInstance().recruitment.editApplicantStatus({ Id: model.id, Code: model.code, Name: model.name, Arrangement: model.arrangement }).$promise.then(function (result) {
                    if (result.isSuccess) {
                        Notification.success("Data Successfully Saved");
    ...
    function cancel(model) {
        model['select'] = false;
        var item = $scope.dataApplicantStatus.find(x => x.id === model.id);
        item.select = false;
        model.code = item.code;
        model.name = item.name;
        model.arrangement = item.arrangement;
        let grid = $("#grid").data("kendoGrid");
        grid.refresh();
    }
    ...
    confirm = function (e) {
        ...
        if (e.data && e.data.value) {
            settingService.getInstance().recruitment.deleteApplicantStatus({ Id: $scope.dataDelete.id }).$promise.then(function (result) {
                if (result.isSuccess) {
