var ssgApp = angular.module("ssg.settingRecruitmentWorkingAddressModule", [
    "kendo.directives"
]);
ssgApp.controller("settingRecruitmentWorkingAddressController", function (
    $rootScope,
    $scope,
    $location,
    $stateParams,
    ...
    //API
    async function getListWorkingAddress(option) {
        ...
        var result = await settingService.getInstance().recruitment.getWorkingAddressRecruiments($scope.currentQuery).$promise;
        console.log(result);
        if (result.isSuccess) {
            $scope.dataWorkingAddress = result.object.data;
            var count = (($scope.currentQuery.page - 1) * $scope.currentQuery.limit) + 1;
            $scope.dataWorkingAddress.forEach(x => {
                x["select"] = false;
                x["no"] = count;
                count = count + 1;
            });
            ...
            $scope.dataWorkingAddress.push(nValue);
    ...
    function buildArgs(pageIndex, pageSize) {
        ...
        if ($scope.keyword) {
            predicate.push('(code.contains(@0) or address.contains(@0))');
    ...
    async function findWorkingAddressByCode(code, id) {
        let queryArgs = {
            predicate: '',
            predicateParameters: [],
        ...
        var result = await settingService.getInstance().recruitment.getWorkingAddressRecruimentByCode(queryArgs).$promise;
    ...
    // Get list
    $scope.workingAddressOptions = {
        dataSource: {
            serverPaging: true,
            pageSize: 20,
            transport: {
                read: async function (e) {
                    await getListWorkingAddress(e);
                }
            },
            schema: {
                total: () => { return $scope.total },
                data: () => { return $scope.dataWorkingAddress }
            }
        },
        sortable: false,
        editable: {
    ...
    async function create(model) {
        ...
        if (editExist) {
            ...
            if (errors.length > 0) {
                ...
                var checkCode = await findWorkingAddressByCode(model.code);
                if (checkCode) {
                    Notification.error("Code " + model.code + " has existed in Working Address");
                }
                else{
                    settingService.getInstance().recruitment.createWorkingAddressRecruiment({ Id: model.id, Code: model.code, Address: model.address }).$promise.then(function (result) {
                        if (result.isSuccess) {
    ...
    function checkEdit() {
        var result = false;
        $scope.dataWorkingAddress.forEach(item => {
    ...
    async function save(model) {
        ...
        if (errors.length > 0) {
            ...
            var checkCode = await findWorkingAddressByCode(model.code, model.id);
            if (checkCode) {
                Notification.error("Code " + model.code + " has existed in Working Address");
            }
            else {
                settingService.getInstance().recruitment.updateWorkingAddressRecruiment({ Id: model.id, Code: model.code, Address: model.address }).$promise.then(function (result) {
                    if (result.isSuccess) {
                        Notification.success("Data Successfully Saved");
                        getListWorkingAddress();
                        return true;
                    }
                });
            }
    ...
    function cancel(model) {
        model['select'] = false;
        var item = $scope.dataWorkingAddress.find(x => x.id === model.id);
        item.select = false;
    ...
    confirm = function (e) {
        ...
        if (e.data && e.data.value) {
            settingService.getInstance().recruitment.deleteWorkingAddressRecruiment({ Id: $scope.dataDelete.id }).$promise.then(function (result) {
                if (result.isSuccess) {
                    Notification.success("Data Successfully Saved");
                    getListWorkingAddress();
                    grid.refresh();
    ...
    $scope.export = async function () {
        ...
        if ($scope.keyword) {
            model.predicate = '(code.contains(@0) or address.contains(@0))';
            model.predicateParameters.push($scope.keyword);
        }

        var res = await fileService.getInstance().processingFiles.export({
            type: commonData.exportType.WORKINGADDRESSRECRUITMENT
        }, model).$promise;
        if (res.isSuccess) {
