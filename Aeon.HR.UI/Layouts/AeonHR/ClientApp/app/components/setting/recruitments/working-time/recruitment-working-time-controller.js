var ssgApp = angular.module("ssg.settingRecruitmentWorkingTimeModule", [
    "kendo.directives"
]);
ssgApp.controller("settingRecruitmentWorkingTimeController", function (
    $rootScope,
    $scope,
    $location,
    $stateParams,
    ...
    $scope.total = 0;
    $scope.keyword = '';
    $scope.dataWorkingTime = [];
    //API
    async function getWorkingTime(option) {
        ...
        var result = await settingService.getInstance().recruitment.getWorkingTime($scope.currentQuery).$promise;
        if (result.isSuccess) {
            $scope.dataWorkingTime = result.object.data;
            var count = (($scope.currentQuery.page - 1) *  $scope.currentQuery.limit) + 1;
            if ($scope.dataWorkingTime.length) {
                $scope.dataWorkingTime.forEach(x => {
                    x["select"] = false;
                    x["no"] = count;
                    count = count + 1;
                });
            ...
            $scope.dataWorkingTime.push(nValue);
            $scope.total = result.object.count;
            if (option) {
                option.success($scope.dataWorkingTime);
            }
            else {
                let gridWorkingTime = $("#gridWorkingTime").data("kendoGrid");
                gridWorkingTime.dataSource.read();
                gridWorkingTime.dataSource.page($scope.currentQuery.page);
            }
        }
    }
    ...
    async function findWorkingTimeByCode(code, id) {
        let queryArgs = {
            predicate: '',
            predicateParameters: [],
        ...
        var result = await settingService.getInstance().recruitment.getWorkingTimeRecruimentByCode(queryArgs).$promise;
    ...
    //
    $scope.workingTimeOptions = {
        dataSource: {
            serverPaging: true,
            pageSize: 20,
            transport: {
                read: async function (e) {
                    await getWorkingTime(e);
                }
            },
            schema: {
                total: () => { return $scope.total },
                data: () => { return $scope.dataWorkingTime }
            }
        },
        sortable: false,
        pageable: {
    ...
    async function create(model) {
        ...
        if (editExist) {
            ...
            if (errors.length > 0) {
                ...
                var checkCode = await findWorkingTimeByCode(model.code);
                if (checkCode) {
                    Notification.error("Code " + model.code + " has existed in Working Time");
                }
                else {
                    settingService.getInstance().recruitment.addWorkingTime({ Id: model.id, Code: model.code, Name: model.name }).$promise.then(function (result) {
                        if (result.isSuccess) {
    ...
    function checkEdit() {
        var result = false;
        $scope.dataWorkingTime.forEach(item => {
    ...
    async function save(model) {
        ...
        if (errors.length > 0) {
            ...
            var checkCode = await findWorkingTimeByCode(model.code, model.id);
            if (checkCode) {
                Notification.error("Code " + model.code + " has existed in Working Time");
            }
            else {
                settingService.getInstance().recruitment.editWorkingTime({ Id: model.id, Code: model.code, Name: model.name }).$promise.then(function (result) {
                    if (result.isSuccess) {
                        Notification.success("Data Successfully Saved");
                        getWorkingTime();
                        return true;
    ...
    function loadPageOne() {
        let grid = $("#gridWorkingTime").data("kendoGrid");
    ...
    function cancel(model) {
        model['select'] = false
        var item = $scope.dataWorkingTime.find(x => x.id === model.id);
        item.select = false;
    ...
    confirm = function (e) {
        ...
        if (e.data && e.data.value) {
            settingService.getInstance().recruitment.deleteWorkingTime({ Id: $scope.dataDelete.id }).$promise.then(function (result) {
                if (result.isSuccess) {
                    Notification.success("Data Sucessfully Deleted");
                    getWorkingTime();
                    gridWorkingTime.refresh();
                }
                else {
