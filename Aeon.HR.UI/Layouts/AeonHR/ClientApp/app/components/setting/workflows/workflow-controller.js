var ssgApp = angular.module('ssg.settingWorkFlowModule', ['kendo.directives']);
ssgApp.controller('settingWorkFlowController', function ($rootScope, $scope, $location, appSetting, $stateParams, $state, commonData, moment, Notification, $timeout, recruitmentService, masterDataService, settingService, attachmentFile, $anchorScroll) {
        ...
        $scope.workFlowGridOptions = {
            ...
            },
            {
                ...
                template: function (dataItem) {
                    return `<a class='btn btn-sm default blue-stripe k-grid-addItems' ui-sref="home.workflowszzyyxx.item({referenceValue: '${dataItem.id}'})" ui-sref-opts="{ reload: true }">Edit</a>`;
                }
            }
            ]
        };
    ...
    async function getWorkFlowById() {
        let res = await settingService.getInstance().workflows.getWorkflowTemplateById({ Id }).$promise;
        if (res.isSuccess) {
            $scope.title = 'EDIT WORKFLOW:' + ' ' + res.object.workflowName;
            ...
            if ($scope.widget.startWorkflowConditions) {
                $scope.widget.startWorkflowConditions.setDataSource(new kendo.data.DataSource({
                    data: res.object.workflowData.startWorkflowConditions.map(item => ({ ...item, fieldValues: item.fieldValues.join(', ') })) || [],
                }));
            }
        ...
        columns: [
            {
                ...
                template: `<input type="text" class="k-textbox" style="width: 100%;" ng-model="dataItem.fieldName" autocomplete="off" />`,
            },
            {
                field: "fieldValues",
                title: 'Field Values',
                //headerTemplate: $translate.instant('COMMON_REASON'),
                template: `<input type="text" class="k-textbox" style="width: 100%;" ng-model="dataItem.fieldValues" autocomplete="off" />`,
            },
            {
                ...
                template: `<a class="btn btn-sm default red-stripe" ng-click="removeItem(widget.startWorkflowConditions.dataSource, dataItem)">Delete</a>`,
            }
        ],
        selectable: true,
    };
    ...
    $scope.stepConditionsGridOptions = {
        ...
        columns: [
            ...
            },
            {
                ...
                template: `<a class="btn btn-sm default red-stripe" ng-click="removeItem(widget.stepConditions.dataSource, dataItem)">Delete</a>`,
            }
        ],
        selectable: true,
    };
        ...
        columns: [
            {
                field: "name",
                title: 'Name',
                template: `<input type="text" class="k-textbox" style="width: 100%;" ng-model="dataItem.name" autocomplete="off" />`,
            },
            {
                field: "fieldPattern",
                title: 'Field Pattern',
                template: `<input type="text" class="k-textbox" style="width: 100%;" ng-model="dataItem.fieldPattern" autocomplete="off" />`,
            },
            {
                field: "isRequired",
                title: 'Required',
                template: `<input type="checkbox" name="{{dataItem.uid}}" id="{{dataItem.uid}}" class="k-checkbox" ng-model="dataItem.isRequired">
            ...
            },
            {
                ...
                template: `<a class="btn btn-sm default red-stripe" ng-click="removeItem(widget.restrictedProperties.dataSource, dataItem)">Delete</a>`,
            }
        ],
        selectable: true,
    };
    ...
    $scope.onConfirmFlowType = function () {
        $timeout(function () {
            $scope.widget.confirmWorkFlow.close();
            $state.go('home.workflowszzyyxx.item', { referenceValue: '', itemType: $scope.itemType });
        }, 0);
    }

    $scope.deleteWorkflow = function (step, index) {
        $scope.model.workflowData.steps.splice(index, 1);
        if (step === $scope.currentStep)
            $scope.currentStep = $scope.model.workflowData.steps[Math.min(index, $scope.model.workflowData.steps.length - 1)];
        $scope.model.workflowData.steps.forEach((item, index) => item.stepNumber = index + 1);
    }
    ...
    this.$onInit = async () => {
        if ($state.current.name === 'home.workflowszzyyxx.item') {
            if (Id) {
                await getWorkFlowById();
            }
            else {
                $scope.title = 'Create Workflow';
                if ($state.params.itemType)
                    $scope.model.itemType = $state.params.itemType;
                $scope.addMore();
            }
        } else {
            $scope.widget.confirmWorkFlow = $('#confirmFlowType').kendoDialog({
                title: 'Create New Work Flow Template',
                width: "300px",
                modal: true,
                visible: false,
                closable: true,
