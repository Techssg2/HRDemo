using JobSendMail1STResignation.src.Args;
using JobSendMail1STResignation.src.Enums;
using JobSendMail1STResignation.src.Helpers;
using JobSendMail1STResignation.src.ModelEntity;
...
namespace JobSendMail1STResignation.src
{
    public class SendNotifications
    {
        ...
        public async Task<bool> SendNotification1STResignation(List<string> ReferecenNumbers)
        {
            ...
            var allEmailHistories = GetEmailHistoriesByReferenceNumbers(ReferecenNumbers);
            if (allEmailHistories.Any())
            {
                foreach (var emailHistories in allEmailHistories)
                {
                    ...
                    var userSend = GetUsersByDepartmentAndRole(emailHistories.DepartmentId.Value, emailHistories.DepartmentType, true);
                    if (userSend.Any())
                    {
            ...
            if (emailHistoriesNotSend != null)
            {
                try
                {
                    ...
                    foreach (var grDept in groupByDepartments)
                    {
                        ...
                        var listUserSendEmail = GetUsersByDepartmentAndRole(grDept.Key.DepartmentId.Value, grDept.Key.DepartmentType, true);
            ...
            try
            {
                foreach (var user in users)
                {
                    ...
                    foreach (var emailHis in emailHistories)
                    {
                        ...
                        {
                            //List<EmailHistoryEntity.UserInfo> info = Mapper.Map<List<EmailHistoryEntity.UserInfo>>(JsonConvert.DeserializeObject<List<EmailHistoryEntity.UserInfo>>(emailHis.UserSent));
        ...
        public async Task UpdateInfoEmailHistories(UserEntity user, List<string> referenceNumbers)
        {
            if (referenceNumbers.Any() && user != null)
            {
                ...
                {
                    //var emailHistories = await _uow.GetRepository<EmailHistoryEntity>().GetSingleAsync(x => x.ReferenceNumber == refe);
                    var emailHistories = GetEmailHistoriesByReferenceNumbers(referenceNumbers).FirstOrDefault(x => x.ReferenceNumber == refe);
                    if (emailHistories != null)
                    {
                        ...
                        {
                            //infor = Mapper.Map<List<EmailHistoryEntity.UserInfo>>(JsonConvert.DeserializeObject<List<EmailHistoryEntity.UserInfo>>(emailHistories.UserSent));
                            var info = JsonConvert.DeserializeObject<List<EmailHistoryEntity.UserInfo>>(emailHistories.UserSent)
                            ?.Select(x => new EmailHistoryEntity.UserInfo
                            {
                                UserId = x.UserId,
                                FullName = x.FullName
                        ...
                        //_uow.GetRepository<EmailHistoryEntity>().Update(emailHistories);
                        ...
                        string updateQuery = string.Format(@"
                            ...
                            emailHistories.ID);
                        emailHistoryQuery.ExecuteRunQuery(updateQuery);
                    }
                }
            }
        }
        ...
        private List<EmailHistoryEntity> GetEmailHistoriesByReferenceNumbers(List<string> referenceNumbers)
        {
        ...
        private List<UserEntity> GetUsersByDepartmentAndRole(Guid departmentId, Group departmentType, bool isHeadCount = true)
        {
            List<UserEntity> r_list = new List<UserEntity>();
            try
            {
        ...
        private List<UserEntity> GetUserSendingByDepartment(Guid departmentId, Group departmentType)
        {
            List<UserEntity> r_list = new List<UserEntity>();
            try
            {
            ...
            {
                Utilities.WriteLogError("GetUserSendingByDepartment:" + ex);
            }
            return r_list;
        }
        #endregion
