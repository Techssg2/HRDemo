...
namespace JobSendMailApproverNotificationsV2.src.Helpers
{
    public static class JobDashboardHelper
    {
        public static List<EdocTaskModel> GetJobTasks()
        {
            var jobTasks = new List<EdocTaskModel>();
            ...
            try
            {
                ...
                foreach (var mi in moduleInstances)
                {
                    ...
                    var itemsFromModule = MapObjectList<EdocTaskModel>(items);
            ...
            foreach (var sourceProp in sourceProperties)
            {
                try
                {
                        ...
                        sourceValueType.GetGenericTypeDefinition() == typeof(List<>))
                    {
                        ...
                        if (sourceValue is IEnumerable sourceEnumerable && targetObj is EdocTaskModel)
                        {
                            ...
                            var mappedList = MapObjectList<WorkflowTaskModel>(sourceList);
